...
def esquivar():
    ...
    for i in range(16): 
        ...
        # sensor_ancla = 15
        if state and np.linalg.norm(point)<1.5 and i in [0,1,2]:# 4, 5  detectan algo, comienza la evitación de obstáculos
              print("obstaculo en sensor: ", i)
              plot_map()
              errf = stop_moving()
              while state is True: #gira hasta que el sensor que detectó el obtejo deje de verlo 
                  state, distance, point, detectedObj, _ = sim.readProximitySensor(usensor[i])
                  bresenham(state,i,point)
        ...
        if state and np.linalg.norm(point)<1.5 and i in [5,6,7]:# 2, 3 detectan algo, comienza la evitación de obstáculos
              print("obstaculo en sensor: ", i)
              errf = stop_moving()
              while state is True:
                  state, distance, point, detectedObj, _ = sim.readProximitySensor(usensor[i])
        ...
        #esquivar por la derecha       
        if state and np.linalg.norm(point)<1.5 and i in [2,3,4,5]:# 2, 3 detectan algo, comienza la evitación de obstáculos
              print("obstaculo en sensor: ", i)
              errf = stop_moving()
              while state is True:
                  state, distance, point, detectedObj, _ = sim.readProximitySensor(usensor[i])
